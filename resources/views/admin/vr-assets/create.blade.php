...
@section('title', isset($vrAsset) ? 'Edit VR Asset' : 'Create VR Asset')
@section('page', 'admin-vr-asset-form')

@section('content')
<div class="page-header">
  <h1>{{ isset($vrAsset) ? 'Edit VR Asset' : 'Create New VR Asset' }}</h1>
  <p>Add or update a therapeutic VR experience</p>
</div>

<div class="card">
  <div class="card-body">
    <form method="POST" action="{{ isset($vrAsset) ? route('admin.vr-assets.update', $vrAsset) : route('admin.vr-assets.store') }}" enctype="multipart/form-data">
      @csrf
      @if(isset($vrAsset))
        @method('PUT')
      @endif
      ...
      <div class="form-group">
        ...
        <input type="number" id="difficulty_level" name="difficulty_level" class="form-input" min="1" max="5" required value="{{ old('difficulty_level', $vrAsset->difficulty_level ?? 1) }}">
      ...
      <div class="form-group">
        ...
        @if(isset($vrAsset) && $vrAsset->image_path)
      ...
      <div class="form-group">
        ...
        @if(isset($vrAsset) && $vrAsset->file_path)
      ...
      <div class="form-group">
        <label class="checkbox-label">
          <input type="checkbox" name="is_active" value="1" {{ old('is_active', $vrAsset->is_active ?? true) ? 'checked' : '' }}>
          <span>Active (Available to users)</span>
        </label>
      </div>

      <div class="form-actions">
        <button type="submit" class="btn btn-primary">{{ isset($vrAsset) ? 'Update Asset' : 'Create Asset' }}</button>
